// Raw employee dataset structure provided
const employeeSeeds = [
...
export const generateDataset = (targetCount = 20) => {
  const data = [];
  
  for (let i = 0; i < targetCount; i++) {
    // Loop over baseline rows to preserve structure at any scale
    const seed = employeeSeeds[i % employeeSeeds.length];
    const uniqueId = i + 1;
    
    data.push({
      ...seed,
      id: uniqueId,
      // Modify names slightly when expanding to 20k rows to create unique rows
      firstName: i >= employeeSeeds.length ? `${seed.firstName} #${Math.floor(i / employeeSeeds.length)}` : seed.firstName,
      email: i >= employeeSeeds.length ? `${seed.firstName.toLowerCase()}.${uniqueId}@company.com` : seed.email
    });
  }
...
// Parameterized data payload. Kept at 20 rows, easily scales up to 20000 rows.
export const mockData = generateDataset(20000);